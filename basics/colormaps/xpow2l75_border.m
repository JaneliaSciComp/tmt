function im = xpow2l75_border(z, z_mag_lim, z_mag_thresh)

% z is mxn of complex, typically a cross-power value or similar
% im is mxnx3, RGB image

if nargin<2 || isempty(z_mag_lim)
  z_mag_lim = max(z, 'all') ;
end
if nargin<3 || isempty(z_mag_thresh)
  z_mag_thresh=0;
end
...
z_mag=min(max(z_mag,0),z_mag_lim);
f_mag = z_mag/z_mag_lim ;
im_l=75*f_mag;
...
im_ab=repmat(f_mag,[1 1 2]).*im_ab_r_max;
%im_ab=im_ab_r_max;
