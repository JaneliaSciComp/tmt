function result = interpolate_chain(xyz_from_node_index, maximum_spacing)
    % Get length of chain    
    ...
    estimated_result_node_count = 1.2 * (estimated_result_edge_count+1) ;  % Pad so we (hopefully) don't need to reallocate array
    ...
    for i = 1 : edge_count ,
        ...
        if edge_length <= maximum_spacing ,
            ...
            new_nodes = new_dxyz .* (1:new_edge_count_for_this_edge)' ;
            result(result_node_count_so_far+1:result_node_count_so_far+new_edge_count_for_this_edge,:) = new_nodes ;
